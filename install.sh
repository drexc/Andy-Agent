...
confirm_kernel_runtime_setup() {
	...
	if prime_agent_prompt_yes_no \
		...
		"Installs uv, Python 3.11, and Andy Agent runtime." \
		"Prepare? [Y/n]"; then
		prime_agent_bootstrap_kernel_on_install=1
		return
	else
...
Finalizing npm install."
		prime_agent_run_quiet_with_animation_steps \
			...
			"Installing Andy Agent" \
			"$npm_install_details" \
			prime_agent_npm_install "$tarball_path" \
				ANDY_AGENT_BOOTSTRAP_TOOLS_ON_INSTALL=1 \
				ANDY_AGENT_BOOTSTRAP_KERNEL_ON_INSTALL=1 \
				ANDY_AGENT_INSTALL_UV=1 \
