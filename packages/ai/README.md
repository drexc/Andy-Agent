<p align="center">
  <a href="https://primeintellect.ai">
    <picture>
      ...
      <img alt="Andy butterfly mark" src="../../assets/brand/andy-butterfly-black.svg" width="88">
    </picture>
  </a>
</p>

<h1 align="center">Andy Agent AI</h1>

<p align="center">
  LLM provider toolkit.
</p>

Release docs use the Andy Agent package name. The source workspace manifest still keeps an inherited package name until the namespace migration is complete.
...
```bash
npm install andy-agent-ai
```

TypeBox exports are re-exported from `andy-agent-ai`: `Type`, `Static`, and `TSchema`.

## Quick Start

```typescript
import { Type, getModel, stream, complete, Context, Tool, StringEnum } from 'andy-agent-ai';
...
```typescript
import { Type, Tool, StringEnum } from 'andy-agent-ai';
...
```typescript
import { stream, validateToolCall, Tool } from 'andy-agent-ai';
...
```typescript
import { readFileSync } from 'fs';
import { getModel, complete } from 'andy-agent-ai';
...
```typescript
import { getModel, streamSimple, completeSimple } from 'andy-agent-ai';
...
```typescript
import { getModel, stream } from 'andy-agent-ai';
...
```typescript
import { getProviders, getModels, getModel } from 'andy-agent-ai';
...
```typescript
import { Model, stream } from 'andy-agent-ai';
...
Use model-level `thinkingLevelMap` to describe model-specific thinking controls. Keys are Andy Agent thinking levels (`off`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`). Missing keys use provider defaults, string values are sent to the provider, and `null` marks a level unsupported.
...
```typescript
import { streamAnthropic, type AnthropicOptions } from 'andy-agent-ai';
...
```typescript
import { getModel, complete, Context } from 'andy-agent-ai';
...
```typescript
import { Context, getModel, complete } from 'andy-agent-ai';
...
- OAuth login flows are not supported in browser environments. Use the `andy-agent-ai/oauth` entry point in Node.js.
...
```typescript
import { getEnvApiKey } from 'andy-agent-ai';
...
```bash
npx andy-agent-ai login              # interactive provider selection
npx andy-agent-ai login anthropic    # login to specific provider
npx andy-agent-ai list               # list available providers
```
...
The library provides login and token refresh functions via the `andy-agent-ai/oauth` entry point. Credential storage is the caller's responsibility.
...
} from 'andy-agent-ai/oauth';
```

### Login Flow Example

```typescript
import { loginGitHubCopilot } from 'andy-agent-ai/oauth';
...
```typescript
import { getModel, complete } from 'andy-agent-ai';
import { getOAuthApiKey } from 'andy-agent-ai/oauth';
...
- Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `andy-agent-ai`
