...
<h1 align="center">Andy Agent TUI</h1>
...
```typescript
import { TUI, Text, Editor, ProcessTerminal, matchesKey } from "andy-agent-tui";
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "andy-agent-tui";
...
```typescript
import { Container, type Focusable, Input } from "andy-agent-tui";
...
Renders Kitty or iTerm2 terminal graphics when supported. Set `fallbackOnly: true` to show compact image metadata instead; Andy Agent uses this metadata-only mode.
...
```typescript
import { CombinedAutocompleteProvider } from "andy-agent-tui";
...
```typescript
import { matchesKey, Key } from "andy-agent-tui";
...
```typescript
import { visibleWidth, truncateToWidth, wrapTextWithAnsi } from "andy-agent-tui";
...
```typescript
import { matchesKey, Key, truncateToWidth } from "andy-agent-tui";
import type { Component } from "andy-agent-tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "andy-agent-tui";
import type { Component } from "andy-agent-tui";
