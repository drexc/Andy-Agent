...
export async function openFile(filePath: string, options: OpenFileOptions = {}) {
	try {
		...
		if (fileStat && successfulUri) {
			...
			if (options.create && !(originalFilePathForError.endsWith("/") || originalFilePathForError.endsWith("\\"))) {
		...
		try {
			for (const group of vscode.window.tabGroups.all) {
				const existingTab = group.tabs.find(
					(tab) =>
						tab.input instanceof vscode.TabInputText && arePathsEqual(tab.input.uri.fsPath, uriToProcess.fsPath),
				);
				if (existingTab) {
