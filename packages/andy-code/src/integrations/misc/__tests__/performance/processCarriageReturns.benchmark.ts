import { applyRunLengthEncoding, processCarriageReturns, truncateOutput } from "../../extract-text";
...
function calculateStats(durations: number[]) {
	...
	return {
		...
		mean,
		stdDev: Math.sqrt(durations.map((x) => (x - mean) ** 2).reduce((a, b) => a + b, 0) / durations.length),
	};
}
