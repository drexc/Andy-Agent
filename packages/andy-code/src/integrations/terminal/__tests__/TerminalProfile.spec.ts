...
describe("Terminal VS Code terminal profile (#277)", () => {
	...
	describe("createTerminal integration", () => {
		...
		it("merges safe profile env while preserving Andy Code shell-integration vars", () => {
			stubProfiles({
				[Terminal.getPlatformProfileKey(process.platform)]: {
					"Custom Bash": {
						path: "/usr/bin/bash",
	...
	describe("ZDOTDIR injection guard", () => {
		...
		beforeEach(() => {
			zshInitTmpDirSpy = vi.spyOn(ShellIntegrationManager, "zshInitTmpDir").mockReturnValue("/tmp/roo-zdotdir-test");
