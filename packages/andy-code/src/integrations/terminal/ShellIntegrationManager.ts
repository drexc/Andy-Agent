...
export class ShellIntegrationManager {
	...
	public static zshInitTmpDir(env: Record<string, string>): string {
		...
		vscode.workspace.fs
			...
			.then(() => {
				...
				const shellIntegrationPath = ShellIntegrationManager.getShellIntegrationPath("zsh");
	...
	public static zshCleanupTmpDir(terminalId: number): boolean {
		const tmpDir = ShellIntegrationManager.terminalTmpDirs.get(terminalId);
		...
		try {
			...
			ShellIntegrationManager.terminalTmpDirs.delete(terminalId);
	...
	public static clear() {
		ShellIntegrationManager.terminalTmpDirs.forEach((_, terminalId) =>
