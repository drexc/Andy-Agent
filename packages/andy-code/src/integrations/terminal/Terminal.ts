...
export class Terminal extends BaseTerminal {
	...
	constructor(id: number, terminal: vscode.Terminal | undefined, cwd: string) {
		...
		if (terminal) {
			...
			const options: vscode.TerminalOptions = { cwd, name: "Andy Code", iconPath, env };
			...
			if (profileShell?.shellPath) {
				...
				// Andy Code's shell-integration controls authoritative.
				if (profileShell.env) {
	 ...
	 * `.vscode/settings.json` to select an executable for Andy Code to launch.
	 */
	...
	public static getProfileShell(
		...
		if (!pathValue) {
			...
			console.warn(`[Terminal] Terminal profile "${profileName}" has no resolvable "path"; using default terminal.`);
			return undefined;
		}
