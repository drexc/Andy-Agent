...
import type { RooTerminal, RooTerminalProvider } from "./types";
...
export class TerminalRegistry {
	...
	public static initialize() {
		...
		const closeDisposable = vscode.window.onDidCloseTerminal((vsceTerminal) => {
			const terminal = TerminalRegistry.getTerminalByVSCETerminal(vsceTerminal);
		...
		TerminalRegistry.disposables.push(closeDisposable);

		try {
			const startDisposable = vscode.window.onDidStartTerminalShellExecution?.(
				async (e: vscode.TerminalShellExecutionStartEvent) => {
					const terminal = TerminalRegistry.getTerminalByVSCETerminal(e.terminal);
			...
			if (startDisposable) {
				TerminalRegistry.disposables.push(startDisposable);
			}
			...
			if (endDisposable) {
				TerminalRegistry.disposables.push(endDisposable);
			}
		} catch (error) {
	...
	public static createTerminal(cwd: string, provider: RooTerminalProvider): RooTerminal {
		...
		if (provider === "vscode") {
			newTerminal = new Terminal(TerminalRegistry.nextTerminalId++, undefined, cwd);
		} else {
			newTerminal = new ExecaTerminal(TerminalRegistry.nextTerminalId++, cwd);
		}

		TerminalRegistry.terminals.push(newTerminal);
	...
	public static async getOrCreateTerminal(
		...
		if (!terminal) {
			terminal = TerminalRegistry.createTerminal(cwd, provider);
		}
	...
	public static getUnretrievedOutput(id: number): string {
		return TerminalRegistry.getTerminalById(id)?.getUnretrievedOutput() ?? "";
	}
	...
	public static isProcessHot(id: number): boolean {
		return TerminalRegistry.getTerminalById(id)?.process?.isHot ?? false;
	}
	...
	public static getTerminals(busy: boolean, taskId?: string): RooTerminal[] {
		return TerminalRegistry.getAllTerminals().filter((t) => {
	...
	public static cleanup() {
		...
		TerminalRegistry.disposables.forEach((disposable) => disposable.dispose());
	...
	public static closeIdleTerminals(): void {
		TerminalRegistry.terminals = TerminalRegistry.terminals.filter((t) => {
	...
	public static releaseTerminalsForTask(taskId: string): void {
		TerminalRegistry.terminals.forEach((terminal) => {
	...
	private static getAllTerminals(): RooTerminal[] {
		TerminalRegistry.terminals = TerminalRegistry.terminals.filter((t) => !t.isClosed());
		return TerminalRegistry.terminals;
	}

	private static getTerminalById(id: number): RooTerminal | undefined {
		const terminal = TerminalRegistry.terminals.find((t) => t.id === id);
	...
	private static getTerminalByVSCETerminal(vsceTerminal: vscode.Terminal): RooTerminal | undefined {
		const found = TerminalRegistry.terminals.find((t) => t instanceof Terminal && t.terminal === vsceTerminal);

		if (found?.isClosed()) {
			TerminalRegistry.removeTerminal(found.id);
			return undefined;
		}
