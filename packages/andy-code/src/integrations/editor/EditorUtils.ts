...
export class EditorUtils {
	...
	static getFilePath(document: vscode.TextDocument): string {
		let filePath = EditorUtils.filePathCache.get(document);
		if (filePath) {
			return filePath;
		}
		...
		try {
			...
			EditorUtils.filePathCache.set(document, filePath);
			return filePath;
		} catch (error) {
	...
	static getEditorContext(editor?: vscode.TextEditor): EditorContext | null {
		try {
			...
			const effectiveRange = EditorUtils.getEffectiveRange(document, selection);
			...
			const diagnostics = vscode.languages
				...
				.filter((d) => EditorUtils.hasIntersectingRange(effectiveRange.range, d.range))
				.map(EditorUtils.createDiagnosticData);
