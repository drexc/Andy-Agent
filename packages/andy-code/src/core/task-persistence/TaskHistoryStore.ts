...
export class TaskHistoryStore {
		...
		return this.withLock(async () => {
			...
			for (const taskId of onDiskIds) {
				try {
					...
					liveIds.add(taskId);
					if (!options.forceRefresh && this.cache.has(taskId) && this.taskFileMtimes.get(taskId) === mtimeMs) {
						continue;
					}
	...
	private async replayDelegationRepairIntent(): Promise<void> {
		return this.withLock(async () => {
			...
			if (!child || !parent) {
				await this.quarantineDelegationRepairIntent(intent, `missing ${!child ? "child" : "parent"} task record`);
				return;
			}
