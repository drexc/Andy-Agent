...
import { type ProviderSettingsManager, providerProfilesSchema } from "./ProviderSettingsManager";
...
export const importSettings = async ({ providerSettingsManager, contextProxy, customModesManager }: ImportOptions) => {
	...
	const defaultUri = resolveDefaultSaveUri(contextProxy, "lastSettingsExportPath", "andy-code-settings.json", {
		useWorkspace: false,
...
export const exportSettings = async ({ providerSettingsManager, contextProxy }: ExportOptions) => {
	const defaultUri = await resolveDefaultSaveUri(contextProxy, "lastSettingsExportPath", "andy-code-settings.json", {
		useWorkspace: false,
		fallbackDir: path.join(os.homedir(), "Downloads"),
	});
