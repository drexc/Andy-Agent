...
import type { CustomModesManager } from "../CustomModesManager";
import {
	exportSettings,
	importSettings,
...
describe("importExport", () => {
	...
	describe("importSettingsFromPath persistence", () => {
		...
		it("waits for each provider-state write before reporting success", async () => {
			...
			await vi.waitFor(() => expect(mockContextProxy.setValue).toHaveBeenCalledWith("currentApiConfigName", "test"));
	...
	describe("exportSettings", () => {
		...
		it("should export settings to the selected file location", async () => {
			(vscode.window.showSaveDialog as Mock).mockResolvedValue({
				fsPath: "/mock/path/andy-code-settings.json",
			});
			...
			expect(safeWriteJson).toHaveBeenCalledWith("/mock/path/andy-code-settings.json", {
		...
		it("should use the correct default save location", async () => {
			...
			expect(vscode.Uri.file).toHaveBeenCalledWith(path.join("/mock/home", "Downloads", "andy-code-settings.json"));
		});
