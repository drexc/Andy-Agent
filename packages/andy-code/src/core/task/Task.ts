...
import { type DiffStrategy, type ToolParamName, type ToolUse, toolParamNames } from "../../shared/tools";
...
import { createRateLimitClock, type RateLimitClock } from "./RateLimitClock";
...
export class Task extends EventEmitter<TaskEvents> implements TaskLike {
	...
	async ask(
		...
		if (this.abort) {
			...
			throw new Error(`[AndyCode#ask] task ${this.taskId}.${this.instanceId} aborted`);
		}
		...
		while (stack.length > 0) {
			...
			try {
				...
				try {
					...
					drainStreamInBackgroundToFindAllUsage(lastApiReqIndex, this.abort ? "cancelled" : "completed").catch(
						(error) => {
	...
	private async backoffAndAnnounce(retryAttempt: number, error: any): Promise<void> {
		try {
			...
			let exponentialDelay = Math.min(Math.ceil(baseDelay * 2 ** retryAttempt), MAX_EXPONENTIAL_BACKOFF_SECONDS);
