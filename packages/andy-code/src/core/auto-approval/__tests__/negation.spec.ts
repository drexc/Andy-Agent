...
import { baseState, CWD, HOME, type State } from "./fixtures";
...
describe("pattern negation", () => {
	...
	describe("across the two allowlists", () => {
		...
		it("does not let a write-list negation revoke read access", async () => {
			expect(await readDecision({ allowedReadFiles: ["docs/**"], allowedWriteFiles: ["!docs/secret.md"] })).toEqual({
				decision: "approve",
			});
		});
		...
		it("does not let a read-list negation revoke read access granted by the write list", async () => {
			...
			expect(await readDecision({ allowedReadFiles: ["!docs/secret.md"], allowedWriteFiles: ["docs/**"] })).toEqual({
				decision: "approve",
			});
		});
