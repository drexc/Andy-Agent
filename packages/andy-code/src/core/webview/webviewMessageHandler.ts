...
import type { MarketplaceItemType, MarketplaceManager } from "../../services/marketplace";
	...
	switch (message.type) {
		...
		case "abandonSubtaskWithId":
			provider
				...
				.catch((error) =>
					provider.log(`[abandonSubtaskWithId] Failed: ${error instanceof Error ? error.message : String(error)}`),
				);
			break;
		...
		case RouterModelsMessageType.requestRouterModels: {
			...
			if (poeApiKey) {
				if (message?.values?.poeApiKey || message?.values?.poeBaseUrl) {
					await flushModels({ provider: providerIdentifiers.poe, apiKey: poeApiKey, baseUrl: poeBaseUrl }, true);
				}
		...
		case "kimiCodeSignIn": {
			try {
				...
				await provider.postStateToWebview();
				await vscode.env.openExternal(vscode.Uri.parse(device.verificationUriComplete ?? device.verificationUri));
		...
		case "zooCodeSignOut": {
			try {
				...
				provider.log(`Failed to sign out of Andy Code: ${error instanceof Error ? error.message : String(error)}`);
			}
			break;
		}
