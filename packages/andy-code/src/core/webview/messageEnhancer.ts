...
import { type ClineMessage, GlobalState, type ProviderSettings, TelemetryEventName } from "@roo-code/types";
	...
	static async enhanceMessage(options: MessageEnhancerOptions): Promise<MessageEnhancerResult> {
		try {
			...
			if (includeTaskHistoryInEnhance && currentClineMessages && currentClineMessages.length > 0) {
				const taskHistory = MessageEnhancer.extractTaskHistory(currentClineMessages);
				if (taskHistory) {
