...
export async function generateErrorDiagnostics(params: GenerateDiagnosticsParams): Promise<GenerateDiagnosticsResult> {
	...
	try {
		...
		const headerComment =
			"// Please share this file with Andy Code Support (support@ia.v2nethost.cl:3000) to diagnose the issue faster\n" +
		...
		const tempFileName = `andy-diagnostics-${taskId.slice(0, 8)}-${timestamp}.json`;
