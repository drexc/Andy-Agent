...
describe("ClineProvider pending task actions", () => {
		...
		const provider = {
			taskHistoryStore: {
				atomicReadAndUpdate: vi
					.fn()
					.mockRejectedValue(new Error("[TaskHistoryStore] atomicReadAndUpdate: task task-1 not found in cache")),
			},
		} as unknown as ClineProvider;
	...
	it("propagates unrelated store failures", async () => {
		...
		await expect(ClineProvider.prototype.clearPendingTaskAction.call(provider, "task-1", "action-1")).rejects.toThrow(
			"disk unavailable",
		);
	});
});
