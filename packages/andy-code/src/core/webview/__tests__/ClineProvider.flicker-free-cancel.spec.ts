...
vi.mock("../diff/strategies/multi-search-replace", () => ({
	MultiSearchReplaceDiffStrategy: vi
		.fn()
		.mockImplementation(() => ({ getName: () => "test-strategy", applyDiff: vi.fn() })),
}));
...
describe("ClineProvider flicker-free cancel", () => {
	...
	beforeEach(() => {
		...
		vi.mocked(Task).mockImplementation(() => mockTask2 as unknown as Task);
	});
