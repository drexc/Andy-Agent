...
describe("ClineProvider", () => {
	...
	test("resolveWebviewView sets up webview correctly", async () => {
		...
		expect(mockWebviewView.webview.html).toContain("<title>Andy Code</title>");
	});
	...
	test("handles webviewDidLaunch message", async () => {
		...
		const messageHandler = (mockWebviewView.webview.onDidReceiveMessage as ReturnType<typeof vi.fn>).mock.calls[0][0];
	...
	it("saves mode config when updating API configuration", async () => {
		...
		(provider as any).providerSettingsManager = {
			listConfig: vi
				.fn()
				.mockResolvedValue([{ name: "test-config", id: "test-id", apiProvider: providerIdentifiers.anthropic }]),
...
describe("webviewMessageHandler no-floating-promises coverage", () => {
	...
	it("covers changed marketplace error and removal responses", async () => {
		...
		const managerArgument = marketplaceManager as unknown as NonNullable<Parameters<typeof webviewMessageHandler>[2]>;
...
describe("ClineProvider - Comprehensive Edit/Delete Edge Cases", () => {
	...
	describe("Andy Code auth profile sync", () => {
		beforeEach(async () => {
			const { getCachedAndyCodeToken } = await import("../../../services/andy-code-auth");
			vi.mocked(getCachedAndyCodeToken).mockReturnValue("");
		});
		...
		describe("handleAndyCodeCallback", () => {
			...
			it("updates every andy-gateway profile and activates only the active one", async () => {
				...
				await provider.handleAndyCodeCallback("new-token");
