...
import { createRateLimitClock, type RateLimitClock } from "../task/RateLimitClock";
...
import { type AggregatedCosts, aggregateTaskCostsRecursive } from "./aggregateTaskCosts";
import { getNonce } from "./getNonce";
import { getUri } from "./getUri";
import { type PendingEditOperationInput, PendingEditOperationStore } from "./PendingEditOperationStore";
	...
	implements vscode.WebviewViewProvider, TelemetryPropertiesProvider, TaskProviderLike
{
	...
	public readonly latestAnnouncementId = "aug-2026-v3.80.1-gateway-promo-models-fixes"; // v3.80.1 Andy Gateway promo, GLM-5.3-Flash, and reliability fixes
	...
	*/
	...
	public static getVisibleInstance(): ClineProvider | undefined {
		return findLast(Array.from(ClineProvider.activeInstances), (instance) => instance.view?.visible === true);
	}

	public static getAllInstances(): ClineProvider[] {
		return Array.from(ClineProvider.activeInstances);
	}
	...
	private async ensureZooGatewayProfileSeeded(): Promise<void> {
		...
		await this.handleAndyCodeCallback(token);
	}
	...
	async handleAndyCodeCallback(token: string) {
		...
		// must explicitly select Andy Gateway in settings if they want to use it.
		try {
			const { apiConfiguration } = await this.getState();
			...
			const { getAndyCodeBaseUrl } = await import("../../services/andy-code-auth");
			const derivedGatewayBaseUrl = `${getAndyCodeBaseUrl()}/api/gateway/v1`;
	...
	async getStateToPostToWebview({ includeTaskHistory = true }: GetStateOptions = {}): Promise<ExtensionState> {
		...
		try {
			const { isAndyCodeAuthenticated, getCachedAndyCodeUserInfo, getAndyCodeBaseUrl } = await import(
			...
			// Keep the default unauthenticated state if the optional Andy Code auth service is unavailable.
		}
	...
	private logWebviewHiddenDiagnostics(): void {
		...
		this.log(
			`[Andy Code] Webview hidden during active task.\n` +
				`  taskId:       ${task.taskId}\n` +
				`  messageCount: ${task.clineMessages.length}\n` +
				`  stackDepth:   ${this.taskRegistry.length}\n` +
				`  timestamp:    ${new Date().toISOString()}\n` +
				`If the panel appears gray after this, share this log with support@ia.v2nethost.cl:3000`,
		);
	}
			...
			await this.taskHistoryStore.atomicUpdatePair(
				...
				(child) => {
					...
					return {
						...
						pendingAction: child.pendingAction?.actionId === pendingActionId ? undefined : child.pendingAction,
					};
				},
				(parent) => {
	...
	public async abandonSubtask(childTaskId: string): Promise<boolean> {
		...
		if (childHistory.status !== "interrupted") {
			this.log(`[abandonSubtask] Aborting: child ${childTaskId} is not interrupted (status=${childHistory.status})`);
			return false;
		}
