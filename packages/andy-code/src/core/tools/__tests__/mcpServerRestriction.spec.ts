...
import { ensureMcpServerAllowed, getAllowedMcpServersForTask, isMcpServerAllowed } from "../mcpServerRestriction";
...
describe("ensureMcpServerAllowed", () => {
	...
	it("rejects invocation when server is NOT in the populated allowlist", async () => {
		...
		const result = await ensureMcpServerAllowed(task, "use_mcp_tool", "disallowed-server", pushToolResult, toolError);
	...
	it("rejects all invocations when allowlist is empty", async () => {
		...
		const result = await ensureMcpServerAllowed(task, "access_mcp_resource", "any-server", pushToolResult, toolError);
