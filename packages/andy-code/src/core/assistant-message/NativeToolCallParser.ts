...
import { type FileEntry, type ToolName, toolNames } from "@roo-code/types";
...
import { MCP_TOOL_PREFIX, MCP_TOOL_SEPARATOR, normalizeMcpToolName, parseMcpToolName } from "../../utils/mcp-name";
...
export class NativeToolCallParser {
	...
	public static processRawChunk(chunk: {
		...
		let tracked = NativeToolCallParser.rawChunkTracker.get(index);
		...
		if (id && !tracked) {
			...
			NativeToolCallParser.rawChunkTracker.set(index, tracked);
		}
	...
	public static processFinishReason(finishReason: string | null | undefined): ToolCallStreamEvent[] {
		...
		if (finishReason === "tool_calls" && NativeToolCallParser.rawChunkTracker.size > 0) {
			for (const [, tracked] of NativeToolCallParser.rawChunkTracker.entries()) {
				events.push({
					type: "tool_call_end",
					id: tracked.id,
				});
	...
	public static finalizeRawChunks(): ToolCallStreamEvent[] {
		...
		if (NativeToolCallParser.rawChunkTracker.size > 0) {
			for (const [, tracked] of NativeToolCallParser.rawChunkTracker.entries()) {
				if (tracked.hasStarted) {
					events.push({
						type: "tool_call_end",
			...
			NativeToolCallParser.rawChunkTracker.clear();
		}
	...
	public static startStreamingToolCall(id: string, name: string): void {
		NativeToolCallParser.streamingToolCalls.set(id, {
			id,
			name,
	...
	public static clearAllStreamingToolCalls(): void {
		NativeToolCallParser.streamingToolCalls.clear();
	}
	...
	public static hasActiveStreamingToolCalls(): boolean {
		return NativeToolCallParser.streamingToolCalls.size > 0;
	}
	...
	public static processStreamingChunk(id: string, chunk: string): ToolUse | null {
		const toolCall = NativeToolCallParser.streamingToolCalls.get(id);
		if (!toolCall) {
			return null;
		}
	...
	public static finalizeStreamingToolCall(id: string): ToolUse | McpToolUse | null {
		...
		NativeToolCallParser.streamingToolCalls.delete(id);
		...
		switch (name) {
			case "read_file":
				...
				if (partialArgs.files !== undefined) {
					...
					if (filesArray && filesArray.length > 0) {
						...
						nativeArgs = {
							files: NativeToolCallParser.convertFileEntries(filesArray),
				...
				if (!nativeArgs && partialArgs.path !== undefined) {
					nativeArgs = {
						...
						offset: NativeToolCallParser.coerceOptionalNumber(partialArgs.offset),
						limit: NativeToolCallParser.coerceOptionalNumber(partialArgs.limit),
						indentation:
							partialArgs.indentation && typeof partialArgs.indentation === "object"
								? {
										anchor_line: NativeToolCallParser.coerceOptionalNumber(
											partialArgs.indentation.anchor_line,
										),
										max_levels: NativeToolCallParser.coerceOptionalNumber(partialArgs.indentation.max_levels),
										max_lines: NativeToolCallParser.coerceOptionalNumber(partialArgs.indentation.max_lines),
										include_siblings: NativeToolCallParser.coerceOptionalBoolean(
											partialArgs.indentation.include_siblings,
										),
										include_header: NativeToolCallParser.coerceOptionalBoolean(
			...
			case "search_and_replace":
				if (
					...
					nativeArgs = {
						...
						old_string: partialArgs.old_string,
						new_string: partialArgs.new_string,
						replace_all: NativeToolCallParser.coerceOptionalBoolean(partialArgs.replace_all),
					};
				}
				break;
			...
			case "list_files":
				if (partialArgs.path !== undefined) {
					nativeArgs = {
						path: partialArgs.path,
						recursive: NativeToolCallParser.coerceOptionalBoolean(partialArgs.recursive),
					};
				}
				break;
	...
	public static parseToolCall<TName extends ToolName>(toolCall: {
		...
		if (typeof toolCall.name === "string") {
			...
			if (normalizedName.startsWith(mcpPrefix)) {
				...
				return NativeToolCallParser.parseDynamicMcpTool({ ...toolCall, name: normalizedName });
			}
		}
			...
			switch (resolvedName) {
				case "read_file":
					...
					if (!nativeArgs && args.path !== undefined) {
						nativeArgs = {
							path: args.path,
							mode: args.mode,
							offset: NativeToolCallParser.coerceOptionalNumber(args.offset),
							limit: NativeToolCallParser.coerceOptionalNumber(args.limit),
							indentation:
								args.indentation && typeof args.indentation === "object"
									? {
											anchor_line: NativeToolCallParser.coerceOptionalNumber(args.indentation.anchor_line),
											max_levels: NativeToolCallParser.coerceOptionalNumber(args.indentation.max_levels),
											max_lines: NativeToolCallParser.coerceOptionalNumber(args.indentation.max_lines),
											include_siblings: NativeToolCallParser.coerceOptionalBoolean(
												args.indentation.include_siblings,
											),
											include_header: NativeToolCallParser.coerceOptionalBoolean(
												args.indentation.include_header,
											),
										}
									: undefined,
						} as NativeArgsFor<TName>;
				...
				case "search_and_replace":
					if (args.file_path !== undefined && args.old_string !== undefined && args.new_string !== undefined) {
						nativeArgs = {
							...
							replace_all: NativeToolCallParser.coerceOptionalBoolean(args.replace_all),
						} as NativeArgsFor<TName>;
					}
					break;
				...
				case "list_files":
					if (args.path !== undefined) {
						nativeArgs = {
							path: args.path,
							recursive: NativeToolCallParser.coerceOptionalBoolean(args.recursive),
						} as NativeArgsFor<TName>;
					}
					break;
