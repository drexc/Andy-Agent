...
import type { McpToolUse, ToolParamName, ToolResponse, ToolUse } from "../../shared/tools";
...
import { type AttemptCompletionCallbacks, attemptCompletionTool } from "../tools/AttemptCompletionTool";
		...
		case "tool_use": {
			...
			if (!block.partial) {
				...
				try {
					...
					cline.recordToolError(toTelemetryToolName(block.name, isCustomTool, stateExperiments), error.message);
