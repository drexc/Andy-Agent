...
vi.mock("../../tools/validateToolUse", () => ({
	...
	isValidToolName: vi.fn((toolName: string) =>
		["read_file", "write_to_file", "ask_followup_question", "attempt_completion", "use_mcp_tool"].includes(toolName),
	),
}));
...
describe("presentAssistantMessage - Image Handling in Native Tool Calling", () => {
	...
	it("should fail fast when tool_use is missing id (legacy/XML-style tool call)", async () => {
		...
		expect(textBlocks.some((b: any) => String(b.text).includes("XML tool calls are no longer supported"))).toBe(true);
