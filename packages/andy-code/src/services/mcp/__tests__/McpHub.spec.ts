...
import type { ConnectedMcpConnection, DisconnectedMcpConnection, McpConnection, McpHub as McpHubType } from "../McpHub";
import { McpHub, ServerConfigSchema } from "../McpHub";
...
describe("McpHub", () => {
	...
	describe("Windows command wrapping", () => {
		...
		it("should wrap commands with cmd.exe on Windows", async () => {
			...
			StdioClientTransport.mockImplementation((config: any) => {
				...
				expect(config.args).toEqual(["/c", "npx", "-y", "@modelcontextprotocol/server-filesystem", "/test/path"]);
	...
	describe("_initiateOAuthFlow with persistent notification", () => {
		...
		it("should disconnect and flag error when Cancel is pressed after Authenticate is clicked", async () => {
			...
			expect(mockConnection.server.status).toBe("disconnected");
			expect((mcpHub as any).appendErrorMessage).toHaveBeenCalledWith(mockConnection, t("mcp:oauth.flow.cancelled"));
			expect(mockAuthProvider.close).toHaveBeenCalled();
		});
