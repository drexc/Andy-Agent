...
import { SecretStorageService, type StoredMcpOAuthData } from "../SecretStorageService";
...
describe("SecretStorageService", () => {
	...
	describe("saveOAuthData", () => {
		it("should persist data under host and path-based key", async () => {
			...
			expect(context.secrets.store).toHaveBeenCalledWith("mcp.oauth.example.com.L21jcA.data", JSON.stringify(data));
		});
