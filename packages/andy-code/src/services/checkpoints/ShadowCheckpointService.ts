...
import type { CheckpointDiff, CheckpointEventMap, CheckpointResult } from "./types";
...
export abstract class ShadowCheckpointService extends EventEmitter {
	...
	protected static workspaceRepoDir({
		...
		return path.join(globalStorageDir, "checkpoints", ShadowCheckpointService.hashWorkspaceDir(workspaceDir));
	}
	...
	public static async deleteTask({
		...
		const workspaceRepoDir = ShadowCheckpointService.workspaceRepoDir({ globalStorageDir, workspaceDir });
		const branchName = `roo-${taskId}`;
		const git = createSanitizedGit(workspaceRepoDir);
		const success = await ShadowCheckpointService.deleteBranch(git, branchName);

		if (success) {
			console.log(`[${ShadowCheckpointService.name}#deleteTask.${taskId}] deleted branch ${branchName}`);
		} else {
			console.error(`[${ShadowCheckpointService.name}#deleteTask.${taskId}] failed to delete branch ${branchName}`);
		}
	}
	...
	public static async deleteBranch(git: SimpleGit, branchName: string) {
		...
		if (!branches.all.includes(branchName)) {
			console.error(
				`[${ShadowCheckpointService.constructor.name}#deleteBranch] branch ${branchName} does not exist`,
			);
			return false;
		}
		...
		if (currentBranch === branchName) {
			...
			try {
				...
				console.error(
					`[${ShadowCheckpointService.constructor.name}#deleteBranch] failed to delete branch ${branchName}: ${error instanceof Error ? error.message : String(error)}`,
				);
