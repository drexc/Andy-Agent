...
import { formatEmbeddingError, type HttpError, withValidationErrorHandling } from "../shared/validation-helpers";
...
export class OpenRouterEmbedder implements IEmbedder {
			...
			try {
				...
				if (httpError?.status === 429) {
					...
					if (hasMoreAttempts) {
						...
						const baseDelay = INITIAL_DELAY_MS * 2 ** attempts;
	...
	private async updateGlobalRateLimitState(error: HttpError): Promise<void> {
		...
		try {
			...
			const exponentialDelay = Math.min(baseDelay * 2 ** (state.consecutiveRateLimitErrors - 1), maxDelay);
