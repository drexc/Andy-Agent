...
export async function getRipgrepDiagnostic(vscodeAppRoot: string): Promise<string> {
	...
	const lines: string[] = [
		`Andy Code Ripgrep Diagnostic (${new Date().toISOString()})`,
...
/**
 * Registers the `andy-code.showRipgrepDiagnostic` command. Thin wrapper —
...
export function registerRipgrepDiagnosticCommand(): vscode.Disposable {
	const channel = vscode.window.createOutputChannel("Andy Code Ripgrep Diagnostic");
	const command = vscode.commands.registerCommand(getCommand("showRipgrepDiagnostic"), async () => {
		...
		await vscode.window.showInformationMessage("Andy Code: ripgrep diagnostic copied to clipboard.");
	});
