...
import { getRipgrepDiagnostic, registerRipgrepDiagnosticCommand, type trySpawnRipgrep } from "../diagnostic";
...
describe("registerRipgrepDiagnosticCommand", () => {
	...
	it("creates an output channel and registers the command", () => {
		...
		expect(vscode.window.createOutputChannel).toHaveBeenCalledWith("Andy Code Ripgrep Diagnostic");
	...
	it("clears, appends, and shows the channel when the command runs", async () => {
		...
		expect(mockChannel.appendLine).toHaveBeenCalledWith(expect.stringContaining("Andy Code Ripgrep Diagnostic"));
	...
	it("writes the report to the clipboard and shows a toast when the command runs", async () => {
		...
		expect(vscode.env.clipboard.writeText).toHaveBeenCalledWith(
			expect.stringContaining("Andy Code Ripgrep Diagnostic"),
		);
		expect(vscode.window.showInformationMessage).toHaveBeenCalledWith(
			"Andy Code: ripgrep diagnostic copied to clipboard.",
		);
	});
});
