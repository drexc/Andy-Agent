import { type OrganizationAllowList, type ProviderSettings, providerIdentifiers } from "@roo-code/types";
...
export class ProfileValidator {
	public static isProfileAllowed(profile: ProviderSettings, allowList: OrganizationAllowList): boolean {
		...
		if (!ProfileValidator.isProviderAllowed(profile.apiProvider, allowList)) {
			return false;
		}

		const modelId = ProfileValidator.getModelIdFromProfile(profile);

		if (!modelId) {
			return allowList.providers[profile.apiProvider]?.allowAll === true;
		}

		return ProfileValidator.isModelAllowed(profile.apiProvider, modelId, allowList);
	}
