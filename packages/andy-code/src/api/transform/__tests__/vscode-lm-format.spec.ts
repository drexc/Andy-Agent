...
import { convertToAnthropicRole, convertToVsCodeLmMessages, extractTextCountFromMessage } from "../vscode-lm-format";
...
describe("convertToVsCodeLmMessages", () => {
	...
	it("should log Andy Code branded warning when asObjectSafe fails to parse invalid JSON string", () => {
		const consoleWarnSpy = vi.spyOn(console, "warn").mockImplementation(() => {});
		...
		expect(consoleWarnSpy).toHaveBeenCalledWith(
			"Andy Code <Language Model API>: Failed to parse object:",
			expect.any(Error),
		);
...
describe("extractTextCountFromMessage", () => {
	...
	it("should log Andy Code branded warning when tool call input stringify fails", () => {
		...
		expect(consoleErrorSpy).toHaveBeenCalledWith(
			"Andy Code <Language Model API>: Failed to stringify tool call input:",
			expect.any(Error),
		);
