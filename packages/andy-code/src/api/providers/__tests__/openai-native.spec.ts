...
describe("OpenAiNativeHandler", () => {
	...
	describe("constructor", () => {
		...
		it("should identify itself as Andy Code in request headers", () => {
			(OpenAI as unknown as ReturnType<typeof vitest.fn>).mockClear();
			new OpenAiNativeHandler({
				apiModelId: "gpt-4.1",
				openAiNativeApiKey: "test-key",
			...
			expect(OpenAI).toHaveBeenCalledWith(
				expect.objectContaining({
					defaultHeaders: expect.objectContaining({
						originator: "andy-code",
						"User-Agent": expect.stringContaining(`andy-code/${Package.version}`),
					}),
				}),
			);
		});
			...
			const mockFetch = vitest.fn().mockResolvedValue({
				...
				body: new ReadableStream({
					start(controller) {
						...
						controller.enqueue(
							new TextEncoder().encode('data: {"type":"response.output_text.delta","delta":" Codex"}\n\n'),
						);
						controller.enqueue(
							new TextEncoder().encode('data: {"type":"response.output_text.delta","delta":" Mini!"}\n\n'),
						);
						controller.enqueue(
							new TextEncoder().encode(
		...
		it("should handle codex-mini-latest stream error events", async () => {
			...
			const mockFetch = vitest.fn().mockResolvedValue({
				...
				body: new ReadableStream({
					start(controller) {
						controller.enqueue(
							new TextEncoder().encode('data: {"type":"response.output_text.delta","delta":"Partial"}\n\n'),
						);
						controller.enqueue(
							new TextEncoder().encode(
								'data: {"type":"response.error","error":{"message":"Model overloaded"}}\n\n',
