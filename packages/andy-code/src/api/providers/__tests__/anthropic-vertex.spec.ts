		...
		it("should use adaptive thinking for Claude Opus 4.8", async () => {
			...
			const mockCreate = vitest
				...
				.mockImplementation(async () =>
					asyncStreamFrom([{ type: "message_start", message: { usage: { input_tokens: 10, output_tokens: 5 } } }]),
				);
			(opus48Handler["client"].messages as any).create = mockCreate;

			await opus48Handler.createMessage("You are a helpful assistant", [{ role: "user", content: "Hello" }]).next();
