...
import { ZOO_GATEWAY_DEFAULT_TEMPERATURE, zooGatewayDefaultModelId } from "@roo-code/types";
...
import { classifyGatewayApiError, toGatewayStreamError, ZooGatewayHandler } from "../zoo-gateway";
...
vitest.mock("../../../services/andy-code-auth", () => ({
	getAndyCodeBaseUrl: vitest.fn(() => "https://www.ia.v2nethost.cl:3000"),
...
describe("ZooGatewayHandler", () => {
	...
	describe("constructor", () => {
		...
		it("defaults the gateway base URL from getAndyCodeBaseUrl", () => {
			...
			expect(OpenAI).toHaveBeenCalledWith(
				expect.objectContaining({
					baseURL: "https://www.ia.v2nethost.cl:3000/api/gateway/v1",
				}),
			);
		});
	});
