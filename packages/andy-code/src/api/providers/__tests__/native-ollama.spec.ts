...
describe("NativeOllamaHandler", () => {
	...
	describe("createMessage", () => {
		...
		it("should map reasoningEffort levels to Ollama think values", async () => {
			const cases: Array<[NonNullable<ApiHandlerOptions["reasoningEffort"]>, boolean | "high" | "medium" | "low"]> =
				[
					["low", "low"],
					["medium", "medium"],
					["high", "high"],
	...
	describe("tool calling", () => {
		it("should include tools when tools are provided", async () => {
			...
			const stream = handler.createMessage("System", [{ role: "user" as const, content: "What's the weather?" }], {
				taskId: "test",
				tools,
			});
