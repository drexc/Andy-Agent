// npx vitest run src/api/providers/fetchers/__tests__/andy-gateway.spec.ts

import axios from "axios";

import { getAndyGatewayModels, parseAndyGatewayModel } from "../andy-gateway";

vitest.mock("axios");
vitest.mock("../../../../services/andy-code-auth", () => ({
	getCachedAndyCodeToken: vitest.fn(() => ""),
	getAndyCodeBaseUrl: vitest.fn(() => "https://example.test"),
	resolveAndyGatewaySessionToken: vitest.fn((profileToken?: string) => profileToken || undefined),
}));
const mockedAxios = axios as any;

describe("Andy Gateway Fetchers", () => {
	beforeEach(() => {
		vitest.clearAllMocks();
	});

	describe("getAndyGatewayModels", () => {
		...
		it("forwards the bearer token and timeout, filters non-language models", async () => {
			...
			const models = await getAndyGatewayModels({
				andyGatewayBaseUrl: baseUrl,
		...
		it("skips the request and returns {} when no token is available", async () => {
			const models = await getAndyGatewayModels({ andyGatewayBaseUrl: baseUrl } as any);
	...
	describe("parseAndyGatewayModel", () => {
		it("enables image attachment from Andy Gateway vision tags", () => {
			const result = parseAndyGatewayModel({
