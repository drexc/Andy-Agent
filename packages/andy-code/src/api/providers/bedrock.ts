...
import type { ModelInfo as CacheModelInfo } from "../transform/cache-strategy/types";
...
export class AwsBedrockHandler extends BaseProvider implements SingleCompletionHandler {
	...
	constructor(options: ProviderSettings) {
		...
		const clientConfig: BedrockRuntimeClientConfig = {
			userAgentAppId: `AndyCode#${Package.version}`,
		...
		const proxyUrl = getSystemProxyUrl(typeof clientConfig.endpoint === "string" ? clientConfig.endpoint : undefined);
		if (proxyUrl) {
	...
	override async *createMessage(
		...
		if ((isThinkingExplicitlyEnabled || isThinkingEnabledBySettings) && modelConfig.info.supportsReasoningBudget) {
			...
			if (isAdaptiveThinkingModel) {
				...
				// display: "summarized" surfaces thinking content in Andy Code UI.
