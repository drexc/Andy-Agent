...
describe("registerCommands handlers", () => {
	...
	it("settingsButtonClicked posts both settingsButtonClicked and didBecomeVisible actions", () => {
		handlers["andy-code.settingsButtonClicked"]();
	...
	it("historyButtonClicked posts historyButtonClicked action", () => {
		handlers["andy-code.historyButtonClicked"]();
	...
	it("marketplaceButtonClicked posts marketplaceButtonClicked action", () => {
		handlers["andy-code.marketplaceButtonClicked"]();
	...
	it("acceptInput posts acceptInput message", () => {
		handlers["andy-code.acceptInput"]();
	...
	it("toggleAutoApprove awaits postMessage with toggleAutoApprove action", async () => {
		...
		const handlerPromise = handlers["andy-code.toggleAutoApprove"]() as Promise<unknown>;
		let settled = false;
	...
	it("focusInput awaits postMessage on the registered provider when a sidebar panel is active", async () => {
		...
		const handlerPromise = handlers["andy-code.focusInput"]() as Promise<unknown>;
		let settled = false;
		void handlerPromise.then(() => {
			settled = true;
		});
	...
	it("focusInput does not post when no sidebar panel is active", async () => {
		await handlers["andy-code.focusInput"]();
	...
	it.each([
		{ command: "andy-code.settingsButtonClicked", prefix: "settingsButtonClicked", expectedCalls: 2 },
		{ command: "andy-code.historyButtonClicked", prefix: "historyButtonClicked", expectedCalls: 1 },
		{ command: "andy-code.marketplaceButtonClicked", prefix: "marketplaceButtonClicked", expectedCalls: 1 },
		{ command: "andy-code.acceptInput", prefix: "acceptInput", expectedCalls: 1 },
	])(
	...
	it("toggleAutoApprove logs to outputChannel when postMessageToWebview rejects", async () => {
		...
		await handlers["andy-code.toggleAutoApprove"]();
	...
	it("plusButtonClicked calls evictCurrentTask on the visible provider", async () => {
		...
		await handlers["andy-code.plusButtonClicked"]();
