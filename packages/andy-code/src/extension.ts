...
import { initAndyCodeAuth } from "./services/andy-code-auth";
...
/**
 * Check if we should auto-open the Andy Code sidebar after switching to a worktree.
...
async function checkWorktreeAutoOpen(
	...
	try {
		...
		if (normalizePath(currentPath) === normalizePath(worktreeAutoOpenPath)) {
			...
			outputChannel.appendLine(`[Worktree] Auto-opening Andy Code sidebar for worktree: ${worktreeAutoOpenPath}`);

			// Open the Andy Code sidebar with a slight delay to ensure UI is ready
			setTimeout(async () => {
				try {
					await vscode.commands.executeCommand("andy-code.plusButtonClicked");
				} catch (error) {
...
export async function activate(context: vscode.ExtensionContext) {
	...
	// Initialize Andy Code auth service for extension session token management.
	await initAndyCodeAuth(context);
