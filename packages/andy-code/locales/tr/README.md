...
> Code'un aktif geliştirmesini durdurduktan sonra Andy Code bu projenin
...
Andy Code, Roo Code'un oluşturduğu temel üzerine inşa ediliyor ve bu temeli şunlarla genişletmeye devam ediyor:
...
- **Daha fazla bağlantı seçeneği** — Andy Gateway, Moonshot, Kimi Code, Kenari, Friendli, OpenCode Go ve çok daha fazlası dahil yeni ve genişletilmiş sağlayıcılar.
...
🤖 Yeni Andy Gateway'de bakiyesi olan kullanıcılar GLM-5.3-Flash ve Gemini 3.7 Flash'ı 2 hafta boyunca %50 indirimle, MiniMax M3'ü ise tamamen ÜCRETSİZ deneyebilir. https://ia.v2nethost.cl:3000/models
...
## Andy Code SİZİN İçin Ne Yapabilir?
...
Andy Code, sizin çalışma şeklinize uyum sağlar, tam tersi değil:
...
Daha fazla: [Modları kullanma](https://docs.ia.v2nethost.cl:3000/basic-usage/using-modes) • [Özel modlar](https://docs.ia.v2nethost.cl:3000/advanced-usage/custom-modes)
...
Andy Code uzantısını çalıştırmanın birkaç yolu vardır:

### Geliştirme Modu (F5)

Aktif geliştirme için VSCode'un yerleşik hata ayıklama özelliğini kullanın:

VSCode'da `F5` tuşuna basın (veya **Çalıştır** → **Hata Ayıklamayı Başlat**'a gidin). Bu, Andy Code uzantısının çalıştığı yeni bir VSCode penceresi açacaktır.
...
1.  İlk olarak, VSIX paketini oluşturun:
    ...
    pnpm vsix
    ```
2.  `bin/` dizininde bir `.vsix` dosyası oluşturulur (ör. `bin/andy-code-<version>.vsix`).
