<p align="center">
        ...
        ## Kami adalah Andy Code

> Andy Code melanjutkan pengembangan proyek ini setelah tim Roo menghentikan
...
Andy Code dikembangkan di atas fondasi yang dibuat oleh Roo Code dan terus memperluasnya dengan:
...
- **Lebih banyak cara untuk terhubung** — provider baru dan yang diperluas, termasuk Andy Gateway, Moonshot, Kimi Code, Kenari, Friendli, OpenCode Go, dan banyak lagi.
...
🤖 Coba GLM-5.3-Flash dan Gemini 3.7 Flash dengan diskon 50% selama 2 minggu, dan dapatkan MiniMax M3 sepenuhnya GRATIS bagi pengguna dengan saldo di Andy Gateway baru. https://ia.v2nethost.cl:3000/models
...
## Apa yang Bisa Andy Code Lakukan Untuk ANDA?
...
Andy Code beradaptasi dengan cara Anda bekerja, bukan sebaliknya:
...
Pelajari lebih lanjut: [Menggunakan Mode](https://docs.ia.v2nethost.cl:3000/basic-usage/using-modes) • [Mode Kustom](https://docs.ia.v2nethost.cl:3000/advanced-usage/custom-modes)
...
Ada beberapa cara untuk menjalankan ekstensi Andy Code:

### Mode Pengembangan (F5)

Untuk pengembangan aktif, gunakan debugging bawaan VSCode:

Tekan `F5` (atau buka **Run** → **Start Debugging**) di VSCode. Ini akan membuka jendela VSCode baru dengan ekstensi Andy Code berjalan.
...
1.  Pertama, bangun paket VSIX:
    ...
    pnpm vsix
    ```
2.  File `.vsix` akan dibuat di direktori `bin/` (misalnya, `bin/andy-code-<version>.vsix`).
...
**Harap dicatat** bahwa Andy Code **tidak** membuat pernyataan atau jaminan apapun mengenai kode, model, atau alat lain yang disediakan atau tersedia sehubungan dengan Andy Code, alat pihak ketiga terkait, atau output yang dihasilkan. Anda menanggung **semua risiko** yang terkait dengan penggunaan alat atau output tersebut; alat tersebut disediakan atas dasar **"SEBAGAIMANA ADANYA"** dan **"SEBAGAIMANA TERSEDIA"**. Risiko tersebut dapat mencakup, namun tidak terbatas pada, pelanggaran kekayaan intelektual, kerentanan atau serangan siber, bias, ketidakakuratan, kesalahan, cacat, virus, waktu henti, kehilangan atau kerusakan properti, dan/atau cedera pribadi. Anda sepenuhnya bertanggung jawab atas penggunaan Anda atas alat atau output tersebut (termasuk, namun tidak terbatas pada, legalitas, kesesuaian, dan hasilnya).
