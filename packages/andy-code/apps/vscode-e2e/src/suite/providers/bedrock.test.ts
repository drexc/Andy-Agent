...
suite("Bedrock provider", function () {
	...
	test("Should complete a task end-to-end via AWS Bedrock with AndyCode# user-agent", async () => {
		...
		if (mockServer) {
			// Verify the AWS SDK transmitted the AndyCode# userAgentAppId.
			...
			assert.ok(userAgent.includes("AndyCode#"), `user-agent should contain "AndyCode#" — got: ${userAgent}`)
		} else {
			...
			assert.ok(true, "Task completed successfully via Bedrock with AndyCode# userAgentAppId")
		}
	})
