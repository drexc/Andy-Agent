...
suite("OpenRouter provider", function () {
	...
	test("Should identify as Andy Code in outbound DEFAULT_HEADERS", async () => {
		requests.length = 0
		...
		for (const captured of requests) {
			assert.strictEqual(captured.xTitle, "Andy Code", "X-Title header should identify the extension as Andy Code")
			assert.strictEqual(
				captured.httpReferer,
				"https://github.com/Zoo-Code-Org/Zoo-Code",
				"HTTP-Referer header should point to the Andy Code repository",
			)
			assert.ok(
				captured.userAgent?.startsWith("AndyCode/"),
				`User-Agent should start with "AndyCode/" — got: ${captured.userAgent}`,
			)
		}
	})
})
