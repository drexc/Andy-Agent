...
export async function run(): Promise<void> {
	...
	const extension = vscode.extensions.getExtension<RooCodeTestAPI>("AndyCodeOrganization.andy-code")
	if (!extension) throw new Error("Andy Code extension not found")
	const api = extension.isActive ? extension.exports : await extension.activate()

	await vscode.commands.executeCommand("andy-code.SidebarProvider.focus")
	await poll(async () => api.isReady(), Boolean, "Andy Code webview activation")
	...
	const themeFixture = await poll<WebviewThemeFixture>(
		...
		`${themeId} Andy Code webview theme`,
	)
	await log(`theme-ready=${themeFixture.themeId}`)

	let landmark = "Welcome to Andy Code!"
	if (scene === "chat") {
