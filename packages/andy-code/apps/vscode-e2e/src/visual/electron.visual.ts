...
async function startScene(
	...
	const temporaryRoot = await fs.mkdtemp(path.join(os.tmpdir(), "andy-code-electron-visual-"))
		...
		try {
			...
			await expect
				.poll(() =>
					running.page
						.frames()
						.some((frame) => frame.url().includes("extensionId=AndyCodeOrganization.andy-code")),
				)
				.toBe(true)
			const parentFrame = running.page
				.frames()
				.find((frame) => frame.url().includes("extensionId=AndyCodeOrganization.andy-code"))
			const contentFrame = parentFrame?.childFrames().find((frame) => frame.url().includes("/fake.html"))
			if (!contentFrame) throw new Error("Andy Code webview content frame not found")
			await contentFrame
			...
			if (scenario.webviewSnapshot) {
				const webview = running.page.locator('iframe[src*="extensionId=AndyCodeOrganization.andy-code"]')
