...
## Extension Host Visual Tests

The Playwright Electron suite under `src/visual/` is a small production-parity layer, not a replacement for the broader `webview-ui` story-gallery snapshots.

- Keep the Electron matrix limited to high-value shell integration scenes that require the real VS Code host.
- Run it with `xvfb-run -a pnpm --filter @roo-code/vscode-e2e test:visual:electron`.
- Scene setup belongs in `sceneController.ts` and must use the exported Andy Code test API plus explicit readiness files. Do not gate screenshots with fixed sleeps.
- Every run uses isolated workspace, user-data, and extensions directories and must clean them up on success and failure.
- Identify the Andy Code webview by its extension ID and frame ancestry, not generated frame IDs or names.
- Keep window geometry, theme, locale, fonts, provider responses, pointer position, and focus deterministic.
- Commit only baselines generated by the pinned CI/Xvfb runner. Failure artifacts include the Playwright report and expected/actual/diff images.
- To recover an intentional Electron baseline change, download the failed run's `extension-host-visual-regression` artifact, inspect the relevant `*-actual.png`, copy it to `src/visual/__screenshots__` without the `-actual` suffix, then push and require the repeated job to pass.
- Treat `_electron` compatibility as part of each pinned VS Code/Playwright upgrade because the API is experimental and depends on Electron fuse support.
