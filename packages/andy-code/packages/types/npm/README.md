...
The Andy Code extension exposes an API that can be used by other extensions.
...
> extension rebrand to Andy Code.
...
const extension = vscode.extensions.getExtension<RooCodeAPI>("AndyCodeOrganization.andy-code")
...
await api.startNewTask("Hello, Andy Code API! Let's make a new project...")
...
**NOTE:** To ensure that the `AndyCodeOrganization.andy-code` extension is activated before your extension, add it to the `extensionDependencies` in your `package.json`:

```json
"extensionDependencies": ["AndyCodeOrganization.andy-code"]
```
