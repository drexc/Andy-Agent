...
import { AndyGateway, pickAndyGatewayDefaultModelId } from "../AndyGateway"
...
const extensionStateMock = {
	andyCodeIsAuthenticated: true,
	andyCodeUserEmail: "user@example.com",
	andyCodeUserName: "User",
...
vi.mock("@src/oauth/urls", () => ({
	getAndyCodeAuthUrl: () => "https://ia.v2nethost.cl:3000/dashboard/connect",
}))
...
function buildRouterModels(modelIds: string[]): RouterModels {
	...
	return { "andy-gateway": models } as unknown as RouterModels
}

describe("pickAndyGatewayDefaultModelId", () => {
	it("falls back to the static default when the catalog is empty", () => {
		expect(pickAndyGatewayDefaultModelId([])).toBe("anthropic/claude-sonnet-4")
	})

	it("prefers an exact anthropic/claude-sonnet-4.5 match", () => {
		const result = pickAndyGatewayDefaultModelId([
	...
	it("falls back to claude sonnet 4 when 4.5 is not in the catalog", () => {
		const result = pickAndyGatewayDefaultModelId(["openai/gpt-4o", "anthropic/claude-sonnet-4"])
		expect(result).toBe("anthropic/claude-sonnet-4")
	})

	it("falls back to the first available id when no claude sonnet is present", () => {
		const result = pickAndyGatewayDefaultModelId(["openai/gpt-4o", "google/gemini-2.5-pro"])
		expect(result).toBe("openai/gpt-4o")
	})
})

describe("AndyGateway component", () => {
	const baseProps = {
	...
	it("auto-selects the resolved default model when the profile has no model id", async () => {
		...
		render(
			<AndyGateway
				apiConfiguration={{ apiProvider: providerIdentifiers.andyGateway } as ProviderSettings}
		...
		await waitFor(() => {
			expect(setApiConfigurationField).toHaveBeenCalledWith("andyGatewayModelId", "anthropic/claude-sonnet-4.5")
		})
	})
		...
		render(
			<AndyGateway
				apiConfiguration={
					{
						apiProvider: providerIdentifiers.andyGateway,
						andyGatewayModelId: "anthropic/claude-sonnet-4.5",
					} as ProviderSettings
				}
				setApiConfigurationField={setApiConfigurationField}
				routerModels={buildRouterModels(["anthropic/claude-sonnet-4", "anthropic/claude-sonnet-4.5"])}
	...
	it("renders the sign-in validation error inline when not authenticated", () => {
		const original = extensionStateMock.andyCodeIsAuthenticated
		extensionStateMock.andyCodeIsAuthenticated = false
		try {
			render(
				<AndyGateway
					apiConfiguration={{ apiProvider: providerIdentifiers.andyGateway } as ProviderSettings}
			...
			expect(screen.getByText("settings:validation.andyGatewaySignIn")).toBeInTheDocument()
		} finally {
			extensionStateMock.andyCodeIsAuthenticated = original
		}
	})
})
