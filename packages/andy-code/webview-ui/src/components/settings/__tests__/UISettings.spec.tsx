...
describe("UISettings", () => {
	...
	describe("auto-close Zoo-opened files checkboxes", () => {
		it("renders all three auto-close checkboxes", () => {
			const { getByTestId } = render(
				<UISettings
					...
					autoCloseAndyOpenedFilesAfterUserEdited={false}
					autoCloseAndyOpenedNewFiles={false}
				/>,
			)
			expect(getByTestId("auto-close-andy-opened-files-checkbox")).toBeTruthy()
			expect(getByTestId("auto-close-andy-opened-files-after-user-edited-checkbox")).toBeTruthy()
			expect(getByTestId("auto-close-andy-opened-new-files-checkbox")).toBeTruthy()
		})

		it("autoCloseAndyOpenedFiles checkbox reflects true prop", () => {
			const { getByTestId } = render(<UISettings {...defaultProps} autoCloseAndyOpenedFiles={true} />)
			const checkbox = getByTestId("auto-close-andy-opened-files-checkbox") as HTMLInputElement
			expect(checkbox.checked).toBe(true)
		})

		it("autoCloseAndyOpenedFiles checkbox reflects false prop", () => {
			const { getByTestId } = render(<UISettings {...defaultProps} autoCloseAndyOpenedFiles={false} />)
			const checkbox = getByTestId("auto-close-andy-opened-files-checkbox") as HTMLInputElement
			expect(checkbox.checked).toBe(false)
		})

		it("autoCloseAndyOpenedFiles checkbox defaults to unchecked when prop is unset", () => {
		...
		it("calls setCachedStateField with autoCloseAndyOpenedFiles when toggled", async () => {
			const setCachedStateField = vi.fn()
			const { getByTestId } = render(
				<UISettings
					{...defaultProps}
			...
			const checkbox = getByTestId("auto-close-andy-opened-files-checkbox")
			fireEvent.click(checkbox)
			await waitFor(() => {
				expect(setCachedStateField).toHaveBeenCalledWith("autoCloseAndyOpenedFiles", false)
			})
		})

		it("calls setCachedStateField with autoCloseAndyOpenedFilesAfterUserEdited when toggled", async () => {
			const setCachedStateField = vi.fn()
			const { getByTestId } = render(
				<UISettings
					{...defaultProps}
			...
			const checkbox = getByTestId("auto-close-andy-opened-files-after-user-edited-checkbox")
			fireEvent.click(checkbox)
			await waitFor(() => {
				expect(setCachedStateField).toHaveBeenCalledWith("autoCloseAndyOpenedFilesAfterUserEdited", true)
			})
		})

		it("calls setCachedStateField with autoCloseAndyOpenedNewFiles when toggled", async () => {
			const setCachedStateField = vi.fn()
			const { getByTestId } = render(
				<UISettings
					{...defaultProps}
			...
			const checkbox = getByTestId("auto-close-andy-opened-new-files-checkbox")
			fireEvent.click(checkbox)
			await waitFor(() => {
				expect(setCachedStateField).toHaveBeenCalledWith("autoCloseAndyOpenedNewFiles", true)
			})
		})
	})
})
