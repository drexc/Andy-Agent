...
vi.mock("react-i18next", () => ({
	Trans: ({ i18nKey, components }: { i18nKey: string; components?: Record<string, React.ReactElement> }) => {
		...
		if (i18nKey === "chat:announcement.release.highlight1" && components?.modelsLink) {
			return React.cloneElement(components.modelsLink, undefined, "https://ia.v2nethost.cl:3000/models")
		}
	...
	useAppTranslation: () => ({
		t: (key: string, options?: { version?: string }) => {
			...
			if (key === "chat:announcement.title") {
				return `Andy Code ${options?.version ?? ""} Released`
			}
...
describe("Announcement", () => {
	it("renders the announcement title and highlights", () => {
		...
		expect(screen.getByText("Andy Code 3.80.1 Released")).toBeInTheDocument()
	...
	it("links support users to the Andy Code GitHub repository", () => {
		render(<Announcement hideAnnouncement={vi.fn()} />)

		expect(screen.getByRole("link", { name: "GitHub" })).toHaveAttribute("href", EXTERNAL_LINKS.GITHUB_REPO)
	})

	it("links the models promo to the Andy Code models page", () => {
		render(<Announcement hideAnnouncement={vi.fn()} />)

		expect(screen.getByRole("link", { name: "https://ia.v2nethost.cl:3000/models" })).toHaveAttribute(
			"href",
	...
	it("posts each announcement link to the extension host exactly once", () => {
		...
		fireEvent.click(screen.getByRole("link", { name: "https://ia.v2nethost.cl:3000/models" }))
		...
		expect(vscode.postMessage).toHaveBeenNthCalledWith(3, { type: "openExternal", url: "https://x.com/AndyCodeDev" })
		...
		expect(vscode.postMessage).toHaveBeenNthCalledWith(5, {
			type: "openExternal",
			url: "https://www.reddit.com/r/AndyCode/",
		})
	})
