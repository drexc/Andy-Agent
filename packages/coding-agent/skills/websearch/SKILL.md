...
Get a free API key at https://serper.dev, then run `/login` in Andy Agent,
switch to **MCP Connections**, and choose **Serper (web search)** to paste it.
The key is stored in Andy Agent and made available to this skill automatically.
...
- `ANDY_AGENT_WEBSEARCH_TIMEOUT` - HTTP timeout in seconds (default 45).
- `ANDY_AGENT_WEBSEARCH_NUM_RESULTS` - number of organic results to return (default 5).
