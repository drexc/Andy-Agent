...
export function formatMissingRipgrepMessage(result: ToolUnavailableResult): string {
	...
	switch (result.reason) {
		...
		case "manual_install_required":
			reason = "Andy Agent cannot install this helper automatically in Termux.";
			break;
		...
		case "download_failed": {
			...
			reason = detail
				? `Andy Agent could not install it automatically: ${detail}`
				: "Andy Agent could not install it automatically.";
			break;
		}
	}

	return [
		"ripgrep (rg) is an optional search helper. Without it, model-run file searches may be slower or fail; Andy Agent and subagents remain available.",
		reason,
