...
import asyncio
import os as _andy_agent_os

_andy_agent_os.environ["NO_COLOR"] = "1"
get_ipython().colors = "nocolor"

try:
    import nest_asyncio as _andy_agent_nest_asyncio
    _andy_agent_nest_asyncio.apply()
except Exception:
    pass

try:
    import rlm as _andy_agent_rlm_module
    rlm = _andy_agent_rlm_module.rlm
    import rlm.graft as graft
    import rlm.mcp as mcp
    mcp.install_shutdown_hook()
except Exception as _andy_agent_rlm_error:
    _ANDY_AGENT_RLM_IMPORT_ERROR = str(_andy_agent_rlm_error)

    class _AndyAgentMissingRlm:
        def _raise_missing(self):
            raise RuntimeError(
                "andy-agent-runtime is not installed in this IPython kernel. "
                "Remove ~/.andy/agent/kernel-venv so andy-agent can rebuild it, or set "
                "ANDY_AGENT_KERNEL_PYTHON to a kernel environment with andy-agent-runtime installed. "
                f"Import error: {_ANDY_AGENT_RLM_IMPORT_ERROR}"
            )
...
import importlib as _andy_agent_importlib
import inspect as _andy_agent_inspect
import sys as _andy_agent_sys
import types as _andy_agent_types

class _AndyAgentCallableSkillModule(_andy_agent_types.ModuleType):
    async def __call__(self, *args, **kwargs):
        result = self.run(*args, **kwargs)
        if _andy_agent_inspect.isawaitable(result):
            return await result
        return result

class _AndyAgentUnavailableSkill:
    def __init__(self, name, error):
        self.__name__ = name
        self._andy_agent_import_error = error
    ...
    async def run(self, *args, **kwargs):
        raise RuntimeError(
            ...
            f"Import error: {self._andy_agent_import_error}"
        )
    ...
    def __repr__(self):
        return f"<unavailable Python skill {self.__name__!r}: {self._andy_agent_import_error}>"

def _andy_agent_wrap_skill_module(module):
    run = getattr(module, "run", None)
    if not callable(run):
        return module
    if isinstance(module, _AndyAgentCallableSkillModule):
        return module
    wrapped = _AndyAgentCallableSkillModule(module.__name__)
    wrapped.__dict__.update(module.__dict__)
    try:
        wrapped.__signature__ = _andy_agent_inspect.signature(run)
    except Exception:
        pass
    doc = getattr(run, "__doc__", None)
    if doc:
        wrapped.__doc__ = doc
    _andy_agent_sys.modules[module.__name__] = wrapped
    return wrapped

_ANDY_AGENT_SKILL_IMPORT_ERRORS = {}

for _andy_agent_skill_name in ${JSON.stringify(importNames)}:
    try:
        globals()[_andy_agent_skill_name] = _andy_agent_wrap_skill_module(
            _andy_agent_importlib.import_module(_andy_agent_skill_name)
        )
    except Exception as _andy_agent_skill_error:
        _ANDY_AGENT_SKILL_IMPORT_ERRORS[_andy_agent_skill_name] = str(_andy_agent_skill_error)
        globals()[_andy_agent_skill_name] = _AndyAgentUnavailableSkill(
            _andy_agent_skill_name,
            str(_andy_agent_skill_error),
        )
`.trim();
}
