...
export const REFINEMENT_CUSTOM_TYPE = "andy-agent.refinement";
...
const REFINEMENT_SYSTEM_PROMPT = `You are Andy Agent's /refine continual harness subsystem.
...
skills, and subagent specs that lets Andy Agent improve reusable behavior
...
- The default editable continual harness store is local to the current Andy Agent session. Use it for session-specific progress, active task state, current-run coordination notes, temporary blockers, and project facts that should not affect other sessions.
...
const AUTO_REFINE_REVIEW_SYSTEM_PROMPT = `You are Andy Agent's automatic /refine review gate.
...
export function formatHarnessStateForPrompt(
	...
	const lines = [
		...
		"",
		"Local continual harness entries belong to this Andy Agent session. Global continual harness entries persist across Andy Agent sessions.",
...
export function getRefinementHistory(entries: readonly CustomEntry[]): RefinementResult[] {
	return entries
		.filter(
			(entry) => entry.customType === REFINEMENT_CUSTOM_TYPE || entry.customType === LEGACY_REFINEMENT_CUSTOM_TYPE,
		)
