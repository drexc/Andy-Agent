...
Example extensions for Andy Agent.

## Usage

```bash
# From the repository root, load an extension with --extension
./andy-agent.sh --extension packages/coding-agent/examples/extensions/permission-gate.ts

# Or copy to extensions directory for auto-discovery
cp permission-gate.ts ~/.andy/agent/extensions/
```
...
| Extension | Description |
|-----------|-------------|
| `mac-system-theme.ts` | Syncs the Andy Agent theme with macOS dark/light mode |
