...
On Windows Terminal, Alt+Enter is fullscreen by default. Remap it as described in [Terminal setup](terminal-setup.md) if you want Andy Agent to receive the shortcut.

Configure delivery in [Settings](settings.md) with `steeringMode` and `followUpMode`.

## Sessions

Sessions are saved automatically as flat JSONL files under `~/.andy/agent/sessions/`. Each session header records its working directory, which the session picker uses for project-scoped views.

```bash
andy-agent -c                  # Continue most recent session
andy-agent -r [path|id]        # Browse sessions or resume one directly
andy-agent --no-session        # Ephemeral mode; do not save
andy-agent --fork <path|id>    # Fork a session into a new session file
```
...
- `.andy/agent/SYSTEM.md` for a project
- `~/.andy/agent/SYSTEM.md` globally
...
andy-agent agents
andy-agent list [--all]
andy-agent attach <agent>
andy-agent stop <agent>
andy-agent rename <agent> <name>
andy-agent send <agent> <message>
andy-agent schedule <list|add|cancel>
andy-agent status
andy-agent doctor [--fix]
andy-agent shutdown [--force]
...
andy-agent update [--force]
andy-agent config
```

See [Andy Agent Packages](packages.md) for package sources and security notes.
...
Use `andy-agent session export <file> [output]` to export a session to HTML.
...
```bash
andy-agent --no-extensions -e ./my-extension.ts
```
...
|----------|-------------|
| `ANDY_AGENT_CODING_AGENT_DIR` | Override config directory; default is `~/.andy/agent` |
...
Extensions, skills, prompt templates, themes, and Andy Agent packages remain the primary customization surfaces. They can add project-specific workflows, custom tools and UI, permission policies, provider integrations, and orchestration patterns around the built-in runtime.

Andy Agent preserves MIT attribution to pi-mono for its upstream lineage, but upstream Pi product claims and limitations do not describe the current Andy Agent architecture.
