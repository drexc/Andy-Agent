> Andy Agent can help you use the SDK. Ask it to build an integration for your use case.

# SDK

The SDK provides programmatic access to Andy Agent's capabilities. Use it to embed Andy Agent in other applications, build custom interfaces, or integrate with automated workflows.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.andy/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.andy/agent/extensions/`)
- Project skills:
  - `.andy/agent/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.andy/agent/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.andy/agent/skills/`)
  - `~/.agents/skills/`
...
// Default: uses ~/.andy/agent/auth.json and ~/.andy/agent/models.json
const authStorage = AuthStorage.create();
const modelRegistry = ModelRegistry.create(authStorage);
...
- If you omit `tools`, Andy Agent automatically creates them with the correct `cwd`
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.andy/agent/extensions/`, `.andy/agent/extensions/`, and `settings.json` extension sources.
...
1. Global: `~/.andy/agent/settings.json`
2. Project: `<cwd>/.andy/agent/settings.json`
...
```bash
andy-agent --mode rpc --no-session
```
