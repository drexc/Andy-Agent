> Andy Agent can create themes. Ask it to build one for your setup.
...
Andy Agent loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.andy/agent/themes/*.json`
- Project: `.andy/agent/themes/*.json`
...
On first run, Andy Agent detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.andy/agent/themes
vim ~/.andy/agent/themes/my-theme.json
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/drexc/andy-agent/main/packages/coding-agent/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
...
**Hot reload:** When you edit the currently active custom theme file, Andy Agent reloads it automatically for immediate visual feedback.
...
Andy Agent uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, Andy Agent falls back to the nearest approximation.
