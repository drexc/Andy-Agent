> Andy Agent can create TUI components. Ask it to build one for your use case.
...
Andy Agent shows compact image metadata and does not emit terminal graphics for images.
...
Customize the inline working indicator shown while Andy Agent is streaming a response.
...
// Restore Andy Agent's default spinner
ctx.ui.setWorkingIndicator();
```
