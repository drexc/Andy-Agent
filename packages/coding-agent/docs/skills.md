> Andy Agent can create skills. Ask it to build one for your use case.
...
- [Creating Skills with Andy Agent](#creating-skills-with-andy-agent)
...
- Built-in: `skills/` shipped with the andy-agent package (lowest precedence)

Discovery rules:
- In `~/.andy/agent/skills/` and `.andy/agent/skills/`, direct root `.md` files are discovered as individual skills
...
Andy Agent ships with built-in skills that load by default:
...
```bash
export ANDY_AGENT_WEBSEARCH_TIMEOUT=45
export ANDY_AGENT_WEBSEARCH_NUM_RESULTS=5
```
...
For project-level Claude Code skills, add to `.andy/agent/settings.json`:
...
1. At startup, Andy Agent scans skill locations and extracts names, descriptions, type, and file locations
...
## Creating Skills with Andy Agent

Andy Agent ships with a built-in `skill-creator` skill that teaches the agent both the Agent Skills format and the Python-backed package contract. You can ask for a skill in normal language:

```text
Create a project Python-backed skill named release-audit in
.andy/agent/skills/release-audit. It should expose
...
Use `/reload` to rediscover new or edited skill metadata. Start a fresh Andy Agent session after adding a Python-backed skill so kernel setup can install and import the package.
...
Andy Agent validates skills against the Agent Skills standard. Most issues produce warnings but still load the skill:
