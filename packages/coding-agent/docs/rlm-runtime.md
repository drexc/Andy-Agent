...
Andy Agent gives each agent session a persistent Python REPL kernel and a native recursive sub-agent interface. The Python `rlm` package is a model-facing shim; the TypeScript host owns child execution, persistence, usage accounting, and lifecycle.
...
1. `ANDY_AGENT_KERNEL_PYTHON`, when it has a current `andy-agent-runtime`;
2. `~/.andy/agent/kernel-venv/bin/python`, bootstrapped with `uv`; or
3. the XDG data location when `~/.andy` is not writable.

The managed environment includes Python 3.11, `andy-agent-runtime`, `dill`, and the default Python packages. A bootstrap marker detects stale environments.
