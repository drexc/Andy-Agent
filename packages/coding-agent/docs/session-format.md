...
```
~/.andy/agent/sessions/<session-id>.jsonl
```
...
Sessions can be removed by deleting their `.jsonl` files under `~/.andy/agent/sessions/`.

Andy Agent also supports deleting sessions interactively from `/resume` (select a session and press `Ctrl+D`, then confirm). When available, Andy Agent uses the `trash` CLI to avoid permanent deletion.
...
### Base Message Types (from Andy Agent AI)
...
### Extended Message Types (from Andy Agent Coding Agent)
...
- `fromHook`: `true` if generated by an extension, `false`/`undefined` if generated by Andy Agent (legacy field name)
