...
```bash
andy-agent --mode json "Your prompt"
```

Outputs all session events as JSON lines to stdout. Useful for integrating Andy Agent into other tools or custom UIs.
...
```bash
andy-agent --mode json "List files" 2>/dev/null | jq -c 'select(.type == "message_end")'
```
