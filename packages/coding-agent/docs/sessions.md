...
Andy Agent saves conversations as sessions so you can continue work, branch from earlier turns, and revisit previous paths.

## Session Storage

Sessions auto-save to `~/.andy/agent/sessions/`. Each session is a JSONL file with a tree structure.

```bash
andy-agent --continue          # Continue the most recent session
andy-agent --resume [path|id]  # Browse past sessions or resume one directly
andy-agent --no-session        # Ephemeral mode; do not save
andy-agent --fork <path|id>    # Fork a session file or partial session ID into a new session
```
...
`/resume` opens an interactive session picker for the current project. `andy-agent --resume` opens the same picker at startup, and `andy-agent --resume <path|id>` resumes a specific session.

An invalid ID exits with the closest unambiguous session ID when one is available. To open the picker and send an initial prompt after selecting a session, separate the prompt with `--`: `andy-agent --resume -- "continue this work"`.
...
When available, Andy Agent uses the `trash` CLI for deletion instead of permanently removing files.
...
Named sessions are easier to find in `/resume` and `andy-agent --resume`.
...
When `/tree` switches away from one branch to another, Andy Agent can summarize the abandoned branch and attach that summary at the new position. This preserves important context from the path you left without replaying the whole branch.
